...
class ReplyLogic extends BaseLogic
{
     ...
     * @return array
     */
    public function getReplyListToArrayByArticle($comment_id, $item_num = 0)
    {
        $reply_tree = $this->getReplyListByArticle($comment_id);
        if (empty($reply_tree)) {
            return $reply_tree;
        }
        $reply_flat_list = $this->treeToArray($reply_tree);
     ...
     * @return array
     */
    private function getReplyListByArticle($comment_id, $parent_id = 0, &$result = array())
    {
        $reply_where = array(
            'comment_id' => $comment_id,
            'parent_id' => $parent_id,
        ...
        foreach ($arr as $cm) {
            $thisArr =& $result[];
            $cm['children'] = $this->getReplyListByArticle($comment_id, $cm['reply_id'], $thisArr);
            $thisArr = $cm;
        }
