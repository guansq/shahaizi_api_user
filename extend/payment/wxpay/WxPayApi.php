...
class WxPayApi {
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30) {
        ...
        //如果有配置代理这里就设置代理
        if (WxPayConfig::CURLOPT_PROXY != "0.0.0.0" && WxPayConfig::CURLOPT_PROXYPORT != 0) {
     ...
     * @return 用户的openid
     */
    public function getOpenId($WxPayConfig) {
        //通过code获得openid
        if (!isset($_GET['code'])) {
            //触发微信返回code码
            $baseUrl = ('http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
            $url = self::__CreateOauthUrlForCode($baseUrl, $WxPayConfig::$APPID);
            Header("Location: $url");
            exit();
        } else {
            //获取code码，以获取openid
            $code = $_GET['code'];
            $openId = self::getOpenidFromMp($code, $WxPayConfig);
            return $openId;
        }
    }
     ...
     * @return 返回构造好的url
     */
    public function __CreateOauthUrlForCode($redirectUrl, $appId) {
        $urlObj["appid"] = $appId;
     ...
     * @return openid
     */
    public function getOpenidFromMp($code, $WxPayConfig) {
        require_once( "WxPayConfig.class.php");
        $url = self::__CreateOauthUrlForOpenid($code, $WxPayConfig);
        //初始化curl
        $ch = curl_init();
        //设置超时
     ...
     * @return 请求的url
     */
    private function __CreateOauthUrlForOpenid($code, $WxPayConfig) {
